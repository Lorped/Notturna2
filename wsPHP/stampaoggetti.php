...
<head>
	...
	<title>NOTTURNA - YOUNG BLOOD</title>
...
<?php
	...
	$Mysqlprestampa="SELECT * FROM prestampa";
	$Resultprestampa=mysqli_query($db, $Mysqlprestampa);
	while ($resprestampa=mysqli_fetch_array($Resultprestampa)) {
		for ($i=0; $i<$resprestampa['quantita']; $i++) {

			$idoggetto = $resprestampa['idoggetto'];
			
			$mysqloggetto="SELECT * FROM oggetti WHERE idoggetto = $idoggetto";
			$Resultoggetto=mysqli_query($db, $mysqloggetto);
			$res=mysqli_fetch_array($Resultoggetto);
...
?>
			...
			<div class="list" style="border: 1px solid #000;">
				...
				<!--  <span style="font-size: 6pt; margin-left: 120px;"> <?=$idoggetto?> </span><br> <img src='../tmp/QR<?=$tt?>.png' style="width: 80px;height: 80px;margin-left: 34px;padding-top: 2px;"> -->
				<span style="font-size: 6pt; margin-left: 160px;">Cartellino <?=$idoggetto?> </span><br> <img src='<?=$fileexternal?>' style="margin-left: 130px;padding-top: 2px;">
				</div>
			</div>
<?php
		} 
