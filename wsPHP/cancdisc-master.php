...
if ( isset($postdata) && $idutente != "" && $iddisciplina != "" ) {
  ...
  $MySql = "DELETE from discipline WHERE idutente = $idutente AND iddisciplina = $iddisciplina";
  $Result = mysqli_query($db, $MySql);

  $MySql = "SELECT poteri.idpotere FROM poteri LEFT JOIN poteri_main ON poteri_main.idpotere=poteri.idpotere WHERE idutente = $idutente and iddisciplina = $iddisciplina";
    $Result = mysqli_query($db, $MySql);
  ...
  $Azione = "MASTER rimossa " . $nomedisc;
